...
def drs(
    ...
    for attempt in range(max_retries):
        x = _drs_iterative(n, U_prime, u_max_prime, rng)
        result = x + umin
...
def _drs_iterative(n: int, U: float, umax: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    ...
    while np.any(active):
        ...
        sample = uunifast(k, remaining_U, rng=rng)
